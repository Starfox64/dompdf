...
class Stylesheet
{
    ...
    private function _parse_css($str)
    {
        ...
        $pattern_css_url_fn = self::PATTERN_CSS_URL_FN;
        ...
        $re = <<<EOL
            /
                ...
                # Match @rules followed by ';' or '{' (that are not part of a URL)
                (?<CSS_ATRULE>
                    @(?<CSS_ATRULE_IDENTIFIER>[^\s{]+)\s*
                    (?<CSS_ATRULE_RULE>
                        {{$pattern_css_url_fn}}(?<CSS_ATRULE_URL_MEDIA_QUERY>.*?)
                        |
                        {{$pattern_media_query}}
                        |
                        [^{;]*
                ...
                # Only parse sub-sections if we're in an @rule...
                (?(CSS_ATRULE)
                    #...and if there was a leading '{'
                    (?(CSS_ATRULE_BRACKET)
                        # Parse rulesets and individual @page rules
                        \s*(?<CSS_ATRULE_BODY> (?:(?>[^{}]+) (?<CSS_ATRULE_BODY_BRACKET>{)?
                            (?(CSS_ATRULE_BODY_BRACKET) (?>[^}]*) }) \s*)+?
                        )
                    })
...
EOL;
        ...
        foreach ($matches as $match) {
            if ($match["CSS_ATRULE_IDENTIFIER"] !== "") {
                ...
                switch ($atrule_identifier) {
                    ...
                    case "import":
                        $this->_parse_import($match["CSS_ATRULE_RULE"]);
                        break;
                    ...
                    case "media":
                        ...
                        $media_queries = preg_split("/\s*,\s*/", mb_strtolower(trim($match["CSS_ATRULE_RULE"])));
                        foreach ($media_queries as $media_query) {
                            if (in_array($media_query, $acceptedmedia)) {
                                //if we have a media type match go ahead and parse the stylesheet
                                $this->_parse_sections($match["CSS_ATRULE_BODY"]);
                                break;
                                ...
                                foreach ($media_query_matches as $media_query_match) {
                                    if (empty($media_query_match["CSS_MEDIA_QUERY_TYPE"]) === false) {
                                        $media_query_feature = strtolower($media_query_match["CSS_MEDIA_QUERY_TYPE"]);
                                        $media_query_value = strtolower($media_query_match["CSS_MEDIA_QUERY_OP"]);
                                ...
                                $this->_parse_sections($match["CSS_ATRULE_BODY"], $mq);
                                break;
                            }
                        }
                        break;
                    ...
                    case "page":
                        ...
                        $page_selector = trim($match["CSS_ATRULE_RULE"]);
                        ...
                        if (empty($this->_page_styles[$key])) {
                            $this->_page_styles[$key] = $this->_parse_properties($match["CSS_ATRULE_BODY"]);
                        } else {
                            $this->_page_styles[$key]->merge($this->_parse_properties($match["CSS_ATRULE_BODY"]));
                        }
                        break;

                    case "font-face":
                        $this->_parse_font_face($match["CSS_ATRULE_BODY"]);
                        break;
    ...
    {
        static $pattern = "/(?<CSS_IMPORT_URL>" . self::PATTERN_CSS_URL_FN . "|" . self::PATTERN_CSS_STRING . ")(?<CSS_IMPORT_MEDIA_QUERY>.*)/isx";
        if (!preg_match($pattern, $url, $matches)) {
            return;
        }
        $url = $matches["CSS_IMPORT_URL"];
        
        $accept = true;
        $media_queries = preg_split("/\s*,\s*/", mb_strtolower(trim($matches["CSS_IMPORT_MEDIA_QUERY"])));
        if (count($media_queries) > 0) {
            $accept = false;
            $acceptedmedia = self::$ACCEPTED_GENERIC_MEDIA_TYPES;
            $acceptedmedia[] = $this->_dompdf->getOptions()->getDefaultMediaType();

            //TODO: improve negation support
        ...
        foreach ($sources as $source) {
            if (isset($source["CSS_URL_FN_VALUE"]) && in_array($source["CSS_STRING_VALUE"], ["", "truetype"])) {
                $source_info = [
                    ...
                    "path" => Helpers::build_url($this->_protocol, $this->_base_host, $this->_base_path, $source["CSS_URL_FN_VALUE"]),
                ];
